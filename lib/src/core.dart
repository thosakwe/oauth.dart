...
int _unhexDigit(int v) {
  if (v >= $0 && v <= $9) {
    ...
    throw new FormatException("Invalid character " +
        new String.fromCharCode(v) +
        " when decoding hexadecimal digit");
  }
}
...
class Parameter implements Comparable<Parameter> {
  ...
  int compareTo(Parameter r) {
    ...
    if (res == 0) res = _listCmp(_value, r._value);
    return res;
  }
...
String encodeAuthParameters(Map<String, String> map) {
  return map.keys
      .map((key) =>
          ascii.decode(oauthEncode(key)) +
          "=\"" +
          ascii.decode(oauthEncode(map[key])) +
          "\"")
      .join(", ");
}
...
List<int> computeSignatureBase(String method, Uri url, List<Parameter> params) {
  ...
  sigBase.add($amp);
  sigBase.addAll(
      oauthEncode(params.map((p) => p.encode()).reduce(_foldParameters)));
